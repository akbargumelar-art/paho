      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4">
        ...
        <Card className="flex min-h-0 flex-1 flex-col overflow-hidden bg-slate-950/40">
          ...
          <CardContent className="min-h-0 flex-1 overflow-y-auto p-4 md:p-6">
            {messages.length === 0 ? <div className="flex h-full flex-col items-center justify-center text-center text-muted-foreground"><div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10"><CurrentIcon className={cn("h-7 w-7", currentAgent.tone)} /></div><h2 className="text-base font-semibold text-foreground">Mulai chat project {project.title}</h2><p className="mt-2 max-w-md text-sm">Instruksi, knowledge, dan file project ini akan ikut masuk ke prompt.</p></div> : <div className="mx-auto flex max-w-4xl flex-col gap-5">{messages.map((message) => <div key={message.id} className={cn("flex gap-3", message.role === "user" ? "justify-end" : "justify-start")}>{message.role === "assistant" && <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary"><CurrentIcon className={cn("h-4 w-4", currentAgent.tone)} /></div>}<div className={cn("max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm", message.role === "user" ? "bg-primary text-primary-foreground" : "border border-border bg-card text-card-foreground")}><div className="whitespace-pre-wrap">{message.content}</div>{Boolean(message.attachments?.length) && <div className="mt-3 flex flex-col gap-2">{message.attachments?.map((file) => <a key={file.id} href={attachmentUrl(file.id)} className="rounded-lg border border-border bg-background/60 px-3 py-2 text-xs font-medium text-primary hover:bg-background" download>{file.name} · {(file.size / 1024).toFixed(1)} KB · Download</a>)}</div>}<div className={cn("mt-2 text-[10px] opacity-60", message.role === "user" ? "text-right" : "text-left")}>{new Date(message.createdAt).toLocaleTimeString()}</div></div>{message.role === "user" && <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-700 text-slate-100"><User className="h-4 w-4" /></div>}</div>)}{chatLoading && <div className="flex items-center gap-3 text-sm text-muted-foreground"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-primary"><CurrentIcon className={cn("h-4 w-4", currentAgent.tone)} /></div><div className="rounded-2xl border border-border bg-card px-4 py-3"><Loader2 className="mr-2 inline h-4 w-4 animate-spin" /> {currentAgent.name} menjawab...</div></div>}<div ref={bottomRef} /></div>}
          </CardContent>
        </Card>
