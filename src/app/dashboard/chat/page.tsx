        ...
        <div className="flex items-end gap-3">
          ...
          <div className="flex shrink-0 items-end gap-2">
            <select
              ...
              className="h-11 w-32 rounded-xl border border-input bg-background px-3 text-xs md:w-40"
              disabled={loading}
              title="Pilih agent"
            >
              {agents.map((agent) => (
            ...
            <Button type="submit" size="icon" className="h-11 w-11 rounded-xl" disabled={!input.trim() || loading}>{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</Button>
          </div>
        </div>
      </form>
